...
export async function start_quiz(quiz_id: string): Promise<void> {
    const url = `${BASE_URL}${quiz_id}/start`;
    await fetch(url);
    return;
}

export async function get_quiz_questions(quiz_id: string): Promise<Question[]> {
    const url = `${BASE_URL}${quiz_id}/questions`;
    var response = await fetch(url, { headers: HEADERS });
...
export async function start_quiz_and_get_quiz_questions(quiz_id: string): Promise<Question[]> {
    ...
    return await get_quiz_questions(quiz_id);
}
